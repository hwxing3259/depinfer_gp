...
save(MCMC_CV, file = './beatAML_cv_res/beatAML_linear_original.RData')
...
save(MCMC_CV, file = './beatAML_cv_res/beatAML_linear_logit_Y_with_NA.RData')
...
save(MCMC_CV, file = './beatAML_cv_res/beatAML_linear_X_zeroed_Y_logit.RData')
...
save(MCMC_CV, file = './beatAML_cv_res/beatAML_linear_original_all.RData')
# run this on the new Y dataset, compare the selected proteins with the original
...
save(MCMC_CV, file = './beatAML_cv_res/beatAML_linear_logit_Y_with_NA_all.RData')
# run this on the new X dataset (set NAN=0), see if there is any difference
MCMC_CV <- CV_par(N, burn_in, thin, X_zeroed, Y_new, vec_c0, a0, b0, inclusion_prob, ncore=10, k_fold=3, updated_z = FALSE, old_z = rep(1, P))
save(MCMC_CV, file = './beatAML_cv_res/beatAML_linear_X_zeroed_Y_logit_all.RData')
print(Sys.time()-start_time)



N <- 80 # number of MCMC steps, each step consists of thin times Gibbs update
burn_in<-20 # discard the first burn_in number of samples, results would have lenth = N-burn_in
thin<-2 # number of Gibbs updates in each MCMC step
...
save(MCMC_CV, file = './beatAML_cv_res/beatAML_GP_X_NA_Y_logit.RData')
print(Sys.time()-start_time)
...
MCMC_CV <- CV_nu_par(N=N, burn_in=burn_in, thin=thin, X=GP_X, Y=Y_original, 
                     vec_nu_1=seq(0.01, 0.1, length.out=5), vec_nu_2=c(0.05, 0.1, 0.15, 0.2, 0.25, 0.3), inclusion_prob=.1, back_fit_iter=20, k_fold=3, ncor=10)
save(MCMC_CV, file = './beatAML_cv_res/beatAML_GP_X_NA_Y_original.RData')
print(Sys.time()-start_time)
...
save(MCMC_CV, file = './beatAML_cv_res/beatAML_GP_0_X_NA_Y_logit.RData')
print(Sys.time()-start_time)


start_time = Sys.time()
MCMC_CV <- CV_nu_par_0(N=N, burn_in=burn_in, thin=thin, X=GP_X, Y=Y_original, 
                       vec_nu_1=seq(0.01, 0.3, length.out=5), vec_nu_2=seq(0.01, 0.3, length.out=6), inclusion_prob=.1, back_fit_iter=20, k_fold=3, ncor=12)
save(MCMC_CV, file = './beatAML_cv_res/beatAML_GP_0_X_NA_Y_original.RData')
print(Sys.time()-start_time)


# collected results are saved in beatAML_CV.RData